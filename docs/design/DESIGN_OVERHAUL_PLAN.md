...
**Owner:** Sam · **Created:** 2026-08-24 · **Status:** COMPLETE — D1–D6 done (2026-08-25). The overhaul has landed; what remains is listed in the D6 row below.
...
| 5   | D5 — Screen migration         | **Done** | 2026-08-25 | Batches of 4. **D5a: dashboard, imports, locations, suppliers.** Four raw `<table>`s → DataTable; `StatePanel`-as-loading → `AsyncPanel` + skeletons that hold the table's geometry; `window.confirm` and the capacity upsell → real Dialogs. Three new primitives — `Actions`, `Meta`, `FormSection` — retire 21 inline `style` props and all 12 legacy `var(--text-muted)` reads in this batch. 23 new tests, four of them the standing D5 invariant (no inline style, no `var(--`, no raw table, no `StatePanel`) — add each batch's files to `MIGRATED` in `features.test.tsx` as it lands. 80 web tests green; both themes verified in a browser against real rendered markup. **D5b: alerts, counts, onboarding, reorder.** Alerts became the archive it actually is — a DataTable with an unread/everything view, search and bulk mark-read — and its two fetches split so a dead alert list no longer takes the preferences panel down with it. `Switch` got its first feature consumer (delivery preferences) and a `labelHidden` prop so a settings row can name the setting on the left. Counts' raw `<table>` → DataTable, its badge strip → `StatTile`s, and history moved into its own card with its own retry. Reorder's suggestion list → DataTable with the unit in a `Meta` so the figures keep the tabular face. Three more primitives — `CardIntro`, `Field grow`, `Select compact` — retire the last 20 inline `style` props in the batch. 30 new tests; 110 web, 35 token, 23 mobile green; both themes verified against real rendered markup (that pass caught a duplicated unread control, an over-tinted row action, and an uncapped field). **Also:** `/imports` reached no navigation surface — it is now in the command palette behind the same `item:write` gate the Items page uses. **Also:** D4's promise that Alerts folds into mobile Today is kept — Today opens with unread low-stock alerts, capped at three, with mark-read and a button to the full screen. **D5c: assistant, billing, catalog, operations, support — plus every page outside `features/`, and the alias block.** Assistant's 7 whole-page `StatePanel`s and operations' 8 are gone: each failure is now scoped to the panel that raised it. Catalog's grouped-by-category rendering was **deleted** (Sam's call) — one flat DataTable, category as filter chips whose counts follow the search, and the server-side `search`/`categoryId` params retired in favour of the table's own. Billing was **migrated, not deleted**: the route still answers, and its 39 inline styles became a real pricing layout (`.plan-card`, `SegmentedControl` for the interval, DataTable for the comparison). Operations' invite token moved into a `Dialog` — it is shown once, and a banner above a page you are about to scroll is the wrong place for a secret. Its checkbox groups became real `<fieldset>`/`<legend>`s, fixing nested `<label>`s that were never clickable. New primitives: `Textarea`, `CenteredPage`, `FullPageError`, `FormSection wide`, `DataTable countHidden`, `Segment.label` widened to `ReactNode`. **The four auth pages, the app layout, and the marketing components** carry no inline style and no retired token; `privacy/page.tsx`'s inlined `<style>` block moved into `marketing.css`. **The compatibility-alias block is deleted** — `marketing.css` was its last consumer (71 reads, renamed one alias at a time; every literal verified byte-identical to its ramp counterpart, so the rename is pixel-identical). **The invariant is now DISCOVERY, not an allowlist**: it globs `src/features/**` and `src/app/**` — 35 app screens and 23 marketing files, zero exclusions beyond `src/components/` — so no future file can quietly skip the contract. `StatePanel` has exactly one legitimate caller left (`FullPageError`, for a workspace that failed to load at all) and it lives with the primitives. Both themes verified against real rendered markup, which caught six defects tests could not see, including three filled primaries on one screen and a row count under a table that cannot be narrowed. `.dt-table td` moved to `vertical-align: top` (Sam's carried-forward call) — re-verified across every D5a/D5b table. 289 web tests green. |
| 6   | D6 — Motion and finish        | **Done** | 2026-08-25 | The four curves of §5.3 are **named tokens** (`--ease-swift`, `--ease-calm`, `--dur-swift`, `--dur-calm`, `--dur-commit`), not twenty repeated literals; two off-spec values fixed (a menu ran swift's curve at calm's duration; a row tint ran `120ms ease`). **`commit` had no web implementation at all** — D2 built the native pulse and nothing carried it across — so `@keyframes commit-pulse` now exists and reaches the screen through `StatTile pulse`, on counts' "Accepted N of M": the one figure that moves as you resolve lines. **Route changes** left framer-motion for a CSS animation on the remounting template — the old one animated from React's main thread and re-ran on renders that were not navigations — plus `@view-transition` for the cross-document navigations the app actually makes, exits at 0.8× entrances. **Reanimated, previously imported zero times**, now drives the mobile `Switch` (§5.3's spring, damping 20 / stiffness 150, one shared value driving thumb _and_ track so the pair cannot drift), the `Skeleton` breath, and the shell's session fade. `prefers-reduced-motion` is genuinely exhaustive: the universal selector cannot reach the view-transition pseudo-elements or framer-motion's JS-driven inline styles, so those are handled by name and by `useReducedMotion()`. Two real §5.3 violations found in marketing and fixed — `StockBar` animated **width**, the spec's most explicit prohibition, and `Reveal` ran 800 ms; neither respected reduced motion. **Left undone, deliberately:** (1) the mid-range-Android frame-rate gate — no device, and it cannot be faked; (2) exit animations on Dialog/Sheet/Menu, which unmount instantly today, so "exits 20% faster" is vacuous rather than violated — adding them means reworking the overlay unmount lifecycle and risking D3's verified focus management; (3) full SPA View Transitions, which need React's experimental `<ViewTransition>` or Next's `experimental.viewTransition` flag — an experimental flag this close to launch is Sam's call, and `template.tsx` documents the two-line swap.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
